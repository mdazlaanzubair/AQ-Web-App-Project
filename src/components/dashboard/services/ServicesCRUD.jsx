...
const ServicesCRUD = () => {
  const { services, setSelectedService, deleteService } = useServicesContext();

  const [filteredServices, setFilteredServices] = useState(services);

  const servicesFilter = (search_word) => {
    const matchedItems = services.filter((service) =>
      service.name.toLowerCase().includes(search_word.toLowerCase()) ||
      service.desc.toLowerCase().includes(search_word.toLowerCase())
        ? service
        : ""
    );
  ...
  useEffect(() => setFilteredServices(services), [services]);
    ...
    <section id="dashboard_section" className="py-3">
      <div className="w-full flex flex-row justify-between items-center mb-10 gap-3">
        <input
          ...
          onChange={(e) => servicesFilter(e.target.value)}
        />
        <label
